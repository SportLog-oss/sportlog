...
import { getCurrentRecoveryPct, localDateKey, todayCoachHint } from "@/lib/today";
import { getCompetitions } from "@/lib/data/store";

export default async function PlanningPage() {
  const weekStart = mondayForDate(localDateKey(new Date()));
  const [week, matches, competitions, recoveryPct] = await Promise.all([
    getPlanningWeek(weekStart),
    getPlanningMatches(weekStart),
    getCompetitions(),
    getCurrentRecoveryPct(),
  ]);

  return (
    <PageShell title="Dein Trainingsplan" subtitle="Eine klare Woche. Jede Einheit mit einem Zweck.">
      <WeekPlanner initialWeek={week} initialMatches={matches} initialCompetitions={competitions.filter((competition) => competition.status === "planned")} coachHint={todayCoachHint(recoveryPct)} />
    </PageShell>
  );
}
