                ...
                {sessions.map((session) => {
                  ...
                  return (
                    <div key={session.id} className={clsx("group rounded-xl border border-border bg-surface-raised p-3", session.status === "cancelled" && "opacity-55")}>
                      ...
                      <button onClick={() => setDetailSessionId(session.id)} className={clsx("mt-3 flex w-full items-center gap-2 rounded-lg border px-2.5 py-2 text-left text-xs font-semibold", sessionMatches.some((match) => match.status === "confirmed") ? "border-positive/30 bg-positive/10 text-positive" : "border-border bg-background/30 text-muted hover:border-accent/30 hover:text-accent")}>
                        {sessionMatches.some((match) => match.status === "confirmed") ? <CheckCircle2 size={14} /> : <ClipboardList size={14} />}
                        <span className="min-w-0 flex-1 truncate">{sessionMatches.some((match) => match.status === "confirmed") ? "Plan und Ergebnis ansehen" : sessionMatches.length ? "Plan und Zuordnung prüfen" : "Trainingsinhalt ansehen"}</span>
        ...
        <div className="fixed inset-0 z-50 flex justify-end bg-black/60" onMouseDown={(event) => event.target === event.currentTarget && setDetailSessionId(null)}>
          <aside className="h-full w-full overflow-y-auto border-l border-border bg-surface p-5 shadow-2xl sm:max-w-2xl sm:p-7">
            ...
            <section className="mt-6 rounded-2xl border border-accent/25 bg-accent-soft p-4 sm:p-5">
              <div className="flex items-start gap-3"><ClipboardList size={20} className="mt-0.5 shrink-0 text-accent" /><div><p className="text-xs font-semibold uppercase tracking-wide text-accent">Geplanter Trainingsreiz</p><p className="mt-2 whitespace-pre-line text-sm leading-relaxed">{detailSession.description || "Noch kein genauer Trainingsinhalt hinterlegt."}</p><div className="mt-3 flex flex-wrap gap-2 text-xs"><span className="rounded-full border border-border bg-background/40 px-2.5 py-1">{detailSession.plannedDurationMin ? `${detailSession.plannedDurationMin} Minuten` : "Dauer offen"}</span><span className="rounded-full border border-border bg-background/40 px-2.5 py-1">{INTENSITIES.find((item) => item.value === detailSession.plannedIntensity)?.label ?? "Belastung offen"}</span></div></div></div>
            </section>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onMouseDown={(event) => event.target === event.currentTarget && setModal(null)}>
          <form onSubmit={saveSession} className="max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-2xl border border-border bg-surface p-6 shadow-2xl">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <label className="text-sm sm:col-span-2"><span className="mb-1.5 block text-muted">Trainingsinhalt und Belastungsserie</span><textarea rows={5} value={modal.description} onChange={(event) => setModal({ ...modal, description: event.target.value })} placeholder={'z. B. 3 × (5 × 30 s hart / 1 min locker)\n5 min Serienpause\nFokus: explosiver Antritt, technisch sauber bleiben'} className="w-full resize-y rounded-xl border border-border bg-background px-3 py-2.5 outline-none focus:border-accent" /><span className="mt-1.5 block text-xs leading-relaxed text-muted">Hier gehört der genaue Reiz hinein – Serien, Wiederholungen, Belastungsdauer, Pause und technischer Fokus.</span></label>
            </div>
